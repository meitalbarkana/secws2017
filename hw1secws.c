...
static struct nf_hook_ops nfho_of_ipv6, nfho_to_fw, nfho_from_fw, nfho_others;
...
static int __init my_init_func(){
	...
	/** Takes care of IPv6 packets - blocks them, as said at the lecture **/
	nfth_of_ipv6.hook = hook_func_block;// Function to call when all conditions below are met - blocks the packet
	nfho_of_ipv6.pf = PF_INET6; // Packets of IPv6!
	nfho_of_ipv6.hooknum = NF_INET_PRE_ROUTING; //Called even before deciding if the packet is for/from fw or to/from host1/host2
	nfho_of_ipv6.priority = NF_IP_PRI_FIRST;
	nf_register_hook(&nfho_of_ipv6);
	
	return 0;
}

static void __exit my_exit_func(){
	...
	nf_unregistrt_hook(&nfho_of_ipv6);
